...
class UnitMixOptimizer:
    ...
    def recommend(self, market_data: Dict[str, float | bool | str]) -> UnitMixRecommendation:
        job_core = bool(market_data.get("job_core")) or market_data.get("daytime_population_index", 0) > 120
        family_node = market_data.get("family_age_cohort_index", 0) > 110 and market_data.get("schools_rating", 0) >= 7
        ...
        if job_core and not family_node:
            ...
            rationale = "Proximity to job core favors compact units for young professionals."
            target = "Singles, couples, weekday commuters"
        elif family_node or (outdoor_access and market_data.get("remote_worker_share", 0) > 25):
        ...
        if high_rent_burden or inclusionary:
            ...
            mix["two_bed"] -= 0.05
            mix["three_bed"] -= 0.05
            rationale += " Adjusted for affordability targets and inclusionary requirements."

        # Ensure totals sum to 1.0 after adjustments
        total = sum(mix.values())
        mix = {k: round(v / total, 2) for k, v in mix.items()}
        return UnitMixRecommendation(mix=mix, rationale=rationale, target_audience=target)
